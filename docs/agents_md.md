# AGENTS.md

For information about AGENTS.md, see [this documentation](https://developers.openai.com/codex/guides/agents-md).

## Model-specific sections

Codex also supports inline model-specific sections in `AGENTS.md` via
`<agents-when model="...">...</agents-when>`.

Example:

```md
# AGENTS.md

- These instructions always apply.

<agents-when model="gpt-5.4">
- These instructions are loaded only to the specified agent.
</agents-when>
```

For now, `model="..."` uses exact string matching against the active model
name. Non-matching blocks are omitted before the instructions are sent to the
model.

## Hierarchical agents message

When the `child_agents_md` feature flag is enabled (via `[features]` in `config.toml`), Codex appends additional guidance about AGENTS.md scope and precedence to the user instructions message and emits that message even when no AGENTS.md is present.
